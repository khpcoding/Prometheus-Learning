...
### Histogram
- Client-side sampling of observations and counted in buckets
- Internally actually exposes various counter-type metrics:
  - `_sum` → Sum of total observed values
  - `_count` → Count of total number of observations
  - `_bucket{le="v1"}`
  - `_bucket{le="v2"}`
  - `_bucket{le="+Inf"}` → Each bucket counting number of times observed value was less than or equal to `v1`, `v2`..., `+Inf` respectively
- Unlike gauge, doesn't miss spikes between scrapes because client is sampling all the observations anyway
...
- Sums everything up with same labels only taking in those in `by`, gets filesystem size of all devices on each machine:
  ```promql
  sum by(instance, device)(node_filesystem_size_bytes)
  ```

- Sums everything up with same labels ignoring those in `without`, gets filesystem size of each machine (because this label is not in without):
  ```promql
...
Consider Prometheus 2.2.1 exposing a histogram metric called `prometheus_tsdb_compaction_duration_seconds` that tracks how many seconds compaction takes for the time series database. Obviously it will under the hood expose three counter metrics.
...
- Gets 90th percentile value of compaction seconds over a period of 1d:
  ```promql
  histogram_quantile(
      0.90,
...
### Operators
- **sum**  
  Adds up all the values in a group and returns that as a value for the group.

- **count**  
  Counts the number of time series in a group, and returns it as the value for the group.

- **avg**  
  Returns the average of the values of the time series in the group as the value for the group.
...
- **quantile**  
  ...
  Gets 90th percentile of the system mode CPU usage across the different CPUs in each machine:
  ```promql
...
## PromQL Operators & Functions Cheat Table

| Category              | Operator / Function                  | Description                                                                 |
|-----------------------|--------------------------------------|-----------------------------------------------------------------------------|
| **Arithmetic**        | `+`, `-`, `*`, `/`                   | Basic math operations between vectors or with scalars.                      |
|                       | `%`                                  | Modulo operation.                                                           |
|                       | `^`                                  | Power operator.                                                             |
| **Comparison**        | `==`, `!=`, `>`, `<`                 | Compare values, return 1 (true) or 0 (false).                               |
|                       | `>=`, `<=`                           | Greater/Less than or equal.                                                 |
| **Logical**           | `and`                                | Intersection: return elements present in both vectors.                      |
|                       | `or`                                 | Union: return elements present in either vector.                            |
|                       | `unless`                             | Exclusion: return elements in left vector not in right.                     |
| **Aggregation**       | `sum()`                              | Sum over dimensions.                                                        |
|                       | `avg()`                              | Average over dimensions.                                                    |
|                       | `min()` / `max()`                    | Minimum/Maximum over dimensions.                                            |
|                       | `count()`                            | Count series over dimensions.                                               |
|                       | `count_values()`                     | Count occurrences of each unique value.                                     |
|                       | `stddev()` / `stdvar()`              | Standard deviation / variance.                                              |
|                       | `topk()` / `bottomk()`               | Largest/smallest k elements by value.                                       |
| **Vector Matching**   | `on()`                               | Match only on specified labels.                                             |
|                       | `ignoring()`                         | Match while ignoring specified labels.                                      |
|                       | `group_left`                         | Allow many-to-one matching (left expansion).                                |
|                       | `group_right`                        | Allow one-to-many matching (right expansion).                               |
| **Range Functions**   | `rate()`                             | Per-second average rate of increase over a range.                           |
|                       | `irate()`                            | Instantaneous per-second rate.                                              |
|                       | `increase()`                         | Total increase in counter over a range.                                     |
|                       | `delta()`                            | Difference between first and last value in range.                           |
|                       | `idelta()`                           | Instantaneous difference between last two samples.                          |
|                       | `avg_over_time()`                    | Average over a time range.                                                  |
|                       | `min_over_time()`                    | Minimum over a time range.                                                  |
|                       | `max_over_time()`                    | Maximum over a time range.                                                  |
|                       | `sum_over_time()`                    | Sum over a time range.                                                      |
|                       | `quantile_over_time()`               | Quantile (e.g., median, percentile) over a time range.                      |
|                       | `stddev_over_time()`                 | Standard deviation over a range.                                            |
|                       | `stdvar_over_time()`                 | Standard variance over a range.                                             |
